...
export const getLivres = async (req, res) => {
 ...
 Object.entries(queryFilters).forEach(([key, value]) => {
    switch (key) {
      ...
      case "titre":
        filters.titre = { $regex: value, $options: "i" };
        break;

      case "auteur":
        filters.auteur = { $regex: value, $options: "i" };
        break;
    }
  });
  try {
...
export const livresParGenre = async (req, res) => {
  ...
  try {
    const livres = await Livre.find({ genre: genre });
    res.json(livres);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
export const livresParLangue = async (req, res) => {
  ...
  try {
    const livres = await Livre.find({ langue: new RegExp(langue, 'i') });
    res.json(livres);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
export const livresParStock = async (req, res) => {
  ...
  try {
    const livres = await Livre.find({ stock: { $gte: parseInt(stock) } });
    res.json(livres);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
